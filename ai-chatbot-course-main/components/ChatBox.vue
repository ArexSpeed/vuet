...
<template>
  <div class="fixed bottom-[10px] right-[10px]">
    <button v-show="!open" @click="open = true" class="p-3 bg-blue-500 rounded">
      <IconChat class="w-8 h-8 text-white" />
    </button>
    <div v-if="open" class="box bg-gray-300 dark:bg-gray-800 rounded w-[450px] overflow-hidden">
      <!-- Header -->
      <header class="flex items-center justify-between px-4 bg-gray-200 dark:bg-gray-900">
      ...
      <div class="messages p-4 overflow-y-scroll max-h-[80vh]" ref="messageBox">
        ...
        <ChatBubble v-for="message in messages" :key="message.id" :message="message" :user="getUser(message.userId)"
      ...
      <!-- Footer -->
      <footer class="p-2">
        <input ref="input" class="block w-full px-2 input" type="text" placeholder="Type your message" @keypress.enter="
          $emit('newMessage', {
            id: nanoid(),
            userId: me.id,
            createdAt: new Date(),
